...
async function getAuthenticatedPlayer() {
  ...
  // Admins cannot enter scores — they are not players
  ...
  if ((dbUser as any)?.role === 'admin') {
    throw new Error('Admin accounts cannot enter scores. Admins are not participants in the draw.')
  }
...
export async function addScore(formData: FormData) {
  const { userId } = await getAuthenticatedPlayer()

  const raw = {
    score_value: Number(formData.get('score_value')),
  ...
  // Rule 4: check for duplicate date
  ...
  if (existing) {
    return { error: `You already have a score for ${score_date}. Please edit or delete it instead.` }
  }

  // Rule 1 & 5: if 5 scores exist, delete oldest before inserting
  const { data: currentScores } = await service
    .from('scores')
